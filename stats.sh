...
then
	...
	do
		...
		sortedNumArray=($(for j in "${numArray[@]}"; do echo $j; done | sort -n)) #interates through the array of row values, sorts them, and echoes them out into a new sorted array.
...
then
	...
	#	median=0
	#	medianIndex=0
	#	count=0 #set number count to 0 for average.
	#	sum=0 #set sum to 0 to be divded by count for average. 
	#done < $2
fi
